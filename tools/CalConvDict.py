...
class GenConvDict():
    ...
    def GenConvMatrix(self, transpose=False):
        ...
        # 参数初始化
            ...
            slide = int((SigLength-kernal_length+2*padding)/stride)+1 # 计算滑动窗口数量或滑动步数
            self.SigLengthList.append(slide)
            # 卷积矩阵计算
            con_matrix = np.zeros((in_channel * SigLength, out_channel * slide))
            """
            w1 (1*1024) * (32*256) | 1024 * 8192 
            w2 (32*256) * (64*64) | 8192 * 4096
            w3 (64*64) * (128*32) | 4096 * 4096
            w4 (128*32) * (512*16) | 4096 * 8192
            """
            ...
            if transpose:
                ...
                ConvMatrix.update({Name:con_matrix}) # update()更新或添加键值对
